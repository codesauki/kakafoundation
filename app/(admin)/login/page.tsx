...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.ok) { router.push('/admin'); } else { setError('Invalid email or password.'); setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-navy-gradient flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-2xl text-white font-semibold">Foundation Admin</h1>
          <p className="text-navy-300 text-sm mt-1">Kowa Namu Ne Foundation</p>
        </div>
        <div className="card p-8 shadow-navy-lg">
          <h2 className="font-display text-xl text-navy-800 font-semibold mb-6 text-center">Sign In to Admin Panel</h2>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="input pl-10" placeholder="admin@kowanamunejoundation.org" required />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-navy-300 hover:text-navy-600">
        ...
        <p className="text-center text-navy-400 text-xs mt-6">Only authorised Foundation staff may access this panel.</p>
      </div>
    </div>
  );
}
